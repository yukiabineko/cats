...
#パラメータ日付け時         
        ...
        if params[:date].blank? || !date_valid?(params[:date])
           ...
           redirect_to posts_url
            ...
            @posts = Post.where("serch_date=?",date).limit(10)
...
#ログインされてないので　公開許可ののみ      
       ...
       if params[:category]
           @posts = Post.where(category: params[:category],public:true).order('created_at desc').limit(10)
           @posts2 = []
         if params[:category] == "すべて"
          ...
          if params[:date].blank? || !date_valid?(params[:date])
             ...
             date = Date.parse(params[:date]) 
             @posts = Post.where("serch_date=?",date)
       ...
       #パラメータ投稿者時
         ...
         @posts = Post.where(user_id: user.id,public: true).order('created_at desc').limit(10)  
         @posts2 = []
       else
...
#登録
  ...
  def create
    ...
    if @post.save
       @post.serch_date = Date.parse(@post.created_at.to_s+(60*60*9).to_s)
       @post.save
      redirect_to posts_url  
    else
      render :new
